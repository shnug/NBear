...
namespace NBear.Test.UnitTests.Data
{
    namespace temp_design
    {
        ...
        [NBear.Common.Design.ReadOnly]
        [NBear.Common.Design.MappingName("Order Details Extended")]
        public interface OrderDetailsExtendedSummary : NBear.Common.Design.Entity
        {
            [NBear.Common.Design.PrimaryKey]
            string ProductName { get; }
            decimal UnitPrice { get; }
    ...
    public class GatewayTest
    {
        ...
        public void TestSaveNewEntityWithAutoProperty()
        {
            ...
            gateway.Delete<Category>(newCat.CategoryID);
        }
        ...
        public void TestOracle()
        {
            ...
            TempTable[] objs = oracle.From<TempTable>().ToArrayList<TempTable>(10).ToArray();
            //objs = oracle.GetPageSelector<TempTable>(WhereClip.All, OrderByClip.Default, 10).FindPage(2);
            objs = oracle.From<TempTable>().ToArrayList<TempTable>(10, 10).ToArray();
        }
        ...
        public void TestAccessPageSplit()
        {
            ...
            Message[] objs = access.From<Message>().OrderBy(Message._.ID.Desc).ToArrayList<Message>(5, 5).ToArray();
            objs = access.From<Message>().OrderBy(Message._.ID.Desc).ToArrayList<Message>(5).ToArray();
        }
        ...
        private object DoTestPerformanceRedutionOfPageSplitSqlCache()
        {
            ...
            OrderDetailsExtended[] orders = g.From<OrderDetailsExtended>().ToArrayList<OrderDetailsExtended>(10, 10).ToArray();
            return null;
        }
        ...
        {
            System.Data.DataSet ds = gateway.FromStoredProcedure("CustOrderHist").AddInputParameter("CustomerID", System.Data.DbType.String, 10101).ToDataSet();

            Dictionary<string, object> outValues;// = new Dictionary<string, object>();
            ds = gateway.FromStoredProcedure("SalesByCategoryTest")
                .AddInputParameter("CategoryName", System.Data.DbType.String, "test")
                .AddInputParameter("OrdYear", System.Data.DbType.String, "1998")
                .AddInputOutputParameter("OutP", System.Data.DbType.String, 5, "test")
                .ToDataSet(out outValues);
            Assert.AreEqual("teddy", outValues["OutP"].ToString());

            OrderDetailsExtendedSummary[] objs = gateway.FromStoredProcedure("CustOrdersDetail").AddInputParameter("OrderID", System.Data.DbType.Int32, 10248).ToArrayList<OrderDetailsExtendedSummary>().ToArray();
            Assert.IsTrue(objs.Length > 0);
        }

        [TestMethod]
        public void TestSpecialOrderBy()
        {
            Order[] orders = gateway.From<Order>().OrderBy(new OrderByClip(ExpressionFactory.CreateColumnExpression("newid()", System.Data.DbType.Guid), true)).ToArray<Order>(10, 10);
            Order[] orders2 = gateway.From<Order>().OrderBy(new OrderByClip(ExpressionFactory.CreateColumnExpression("dateadd(day,3,OrderDate)", System.Data.DbType.DateTime), true)).ToArray<Order>(10, 10);
        }
    }
}