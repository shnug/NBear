            ...
            if ((!db.IsBatchConnection) && identyColumn != null && db.DbProvider.SelectLastInsertedRowAutoIDStatement != null)
            {
                ...
                else
                {
                    if (db.DbProvider.SelectLastInsertedRowAutoIDStatement.StartsWith("SELECT SEQ_"))
                    {
                        ...
                        {
                            retVal = db.ExecuteScalar(tran, CommandType.Text, string.Format(db.DbProvider.SelectLastInsertedRowAutoIDStatement, null, table));
                    ...
                    else
                    {
                        ...
                        {
                            retVal = db.ExecuteScalar(tran, CommandType.Text, string.Format(db.DbProvider.SelectLastInsertedRowAutoIDStatement, db.DbProvider.BuildColumnName(identyColumn), db.DbProvider.BuildColumnName(table)));
                            db.ExecuteNonQuery(cmd, tran);
                        }
