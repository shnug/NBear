...
namespace NBear.Common
{
    ...
    public class Entity : MarshalByRefObject
    {
        ...
        /// <summary>
        /// Gets the primary key mapping column pks.
        /// </summary>
        ...
        /// <summary>
        /// Gets the property mapping column pks.
        /// </summary>
        /// <param name="ec">The ec.</param>
        ...
        public virtual void SetPrimaryKeyValues(params object[] pkValues)
        {
            Check.Require(pkValues != null && pkValues.Length > 0, "pkValues could not be null!");

            List<PropertyConfiguration> pks = GetEntityConfiguration().GetPrimaryKeyProperties();

            Check.Require(pks.Count == pkValues.Length, "length of pkProperties and pkValues must equal!");

            for (int i = 0; i < pks.Count; ++i)
            {
                Util.DeepGetProperty(this.GetType(), pks[i].Name).SetValue(this, pkValues[i], null);
            }
        }
        ...
        /// <param name="columnNames">The column pks.</param>
        /// <returns>The values.</returns>
        ...
        /// <summary>
        /// Sets the property unloaded.
        /// </summary>
        /// <param name="propertyName">Name of the property.</param>
        protected void SetPropertyUnloaded(string propertyName)
        {
            lock (queryLoadedProperties)
            {
                if (queryLoadedProperties.Contains(propertyName))
                {
                    queryLoadedProperties.Remove(propertyName);
                }
            }
        }
            ...
            where EntityType : Entity, new()
        {
            ...
            foreach (PropertyConfiguration pc in ec.Properties)
            {
                if ((!pc.IsQueryProperty) || (pc.QueryType == "FkReverseQuery"))
                {
                    string typeName = pc.PropertyMappingColumnType;
                    if (typeName.StartsWith("System.Nullable`1["))
                    {
                        typeName = typeName.Replace("System.Nullable`1[", string.Empty).TrimEnd(']');
                    }
                    Type columnDataType = Util.GetType(typeName);
                    if (pc.IsCompoundUnit)
                    {
                        columnDataType = typeof(string);
                    }
                    else if (columnDataType.IsEnum)
                    {
                        columnDataType = typeof(int);
                    }
                    DataColumn column = new DataColumn(pc.MappingName, columnDataType);
                ...
                for (int i = 0; i < objs.Length; i++)
                {
                    ...
                    foreach (PropertyConfiguration pc in ec.Properties)
                    {
                        if ((!pc.IsQueryProperty) || (pc.QueryType == "FkReverseQuery"))
                        {
                            ...
                            else if (table.Columns[j].DataType == typeof(Guid) && (! (value is Guid)))
                            {
                                row[j] = new Guid(value.ToString());
                            }
        ...
        protected DateTime GetDateTime(DataRow row, int index)
        {
            try
            {
                //if (row.Table.Columns[index].DataType == typeof(DateTime))
                //{
