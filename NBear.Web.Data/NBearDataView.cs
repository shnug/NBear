...
namespace NBear.Web.Data
{
	public class NBearDataView : System.Web.UI.DataSourceView
	{
		...
		protected override IEnumerable ExecuteSelect(DataSourceSelectArguments arguments)
        {
            ...
            sort = gateway.ParseExpressionByMetaData(ec, sort);

			int page = (arguments.MaximumRows > 0 ? (arguments.StartRowIndex / arguments.MaximumRows) + 1 : 1);
            WhereClip where = string.IsNullOrEmpty(owner.FilterExpression) ? WhereClip.All : new WhereClip(gateway.ParseExpressionByMetaData(ec, owner.FilterExpression), null, null, null);
            OrderByClip orderBy = string.IsNullOrEmpty(sort) ? OrderByClip.Default : new OrderByClip(sort);
            Array list;
            if (arguments.MaximumRows > 0)
            {
                FromSection from = (FromSection)GetGatewayMethodInfo("NBear.Data.FromSection From[EntityType]()").MakeGenericMethod(type).Invoke(gateway, null);
                QuerySection query = from.Where(where).OrderBy(orderBy);
                list = (Array)typeof(QuerySection).GetMethod("ToArray", new Type[] { typeof(int), typeof(int) }).MakeGenericMethod(type).Invoke(query, new object[] { arguments.MaximumRows, (page - 1) * arguments.MaximumRows });
            }
            else
            {
