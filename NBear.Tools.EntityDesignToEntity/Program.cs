                    ...
                    if (config != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(config.EntityCodePath))
                        {
                            if (config.EntityCodePath.IndexOf(";") > 0)
                            {
                                ...
                                {
                                    File.WriteAllText(Util.ParseRelativePath(designRootPath, entityCodePath), new CodeGenHelper(config.OutputNamespace, new AdvOptForm()).GenEntitiesEx(ass, config.OutputLanguage.ToLower() == "c#" ? 0 : 1), encoding);
                                }
                            }
                            else
                            {
                                File.WriteAllText(Util.ParseRelativePath(designRootPath, config.EntityCodePath), new CodeGenHelper(config.OutputNamespace, new AdvOptForm()).GenEntitiesEx(ass, config.OutputLanguage.ToLower() == "c#" ? 0 : 1), encoding);
                            }
                        }
                        ...
                        if (!string.IsNullOrEmpty(config.EntityConfigPath))
                        {
                            if (config.EntityConfigPath.IndexOf(";") > 0)
                            {
                                ...
                                {
                                    File.WriteAllText(Util.ParseRelativePath(designRootPath, entityConfigPath), new CodeGenHelper(config.OutputNamespace, new AdvOptForm()).GenEntityConfigurations(ass), System.Text.UnicodeEncoding.UTF8);
                                }
                            }
                            else
                            {
                                File.WriteAllText(Util.ParseRelativePath(designRootPath, config.EntityConfigPath), new CodeGenHelper(config.OutputNamespace, new AdvOptForm()).GenEntityConfigurations(ass), System.Text.UnicodeEncoding.UTF8);
                            }
                        }
